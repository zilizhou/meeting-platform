...
export class AdminService {
  ...
  async warnings(user: AuthUser) {
    ...
    const pack = {
      ...
      precheckMissing: precheckMissing.map((t) => ({
        ...
        level: 'high',
        title: t.title,
        message: `${t.college?.name || ''}：需党委会前置但未关联决议`,
    ...
    pack.monthMissing = [
      ...monthHolding.missingParty.map((c) => ({
        ...
        level: 'high',
        title: `${c.name} · ${monthHolding.label}未按规定召开党委会`,
        message: `${monthHolding.label}应召开党委会 ${monthHolding.requiredParty} 次，当前未见足够排期/召开记录`,
        collegeId: c.collegeId,
  ...
  private renderSchoolBriefingFacts(facts: {
    ...
    const lines = [
      ...
      '',
      '一、总体研判',
      `截至当前，${facts.narrativeUnit}共 ${facts.collegeCount} 所。${facts.monthLabel}党委会已开 ${facts.partyHeldCount} 所、党政联席会议已开 ${facts.jointHeldCount} 所，双会齐全 ${facts.bothOkCount} 所。`,
      ...
      '',
      '二、重点问题',
      `1. ${facts.monthLabel}未按规定召开党委会（${facts.missingPartyCount}）：${missParty}。`,
